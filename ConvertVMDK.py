...
def ConvertQCOW2():
    ...
    os.system("cd " + path)
    getFileName =  "ls *.qcow2 | sed  -e 's/.$//' -e 's/.$//' -e 's/.$//' -e 's/.$//' -e 's/.$//' -e 's/.$//' | tee ConvertRAWfile.txt" #List the files then removes the last 6 chars then puts each line in ConvertRAWfile
    os.system(getFileName)
    try:
        with open("ConvertRAWfile.txt") as fp:
            for i, line in enumerate(fp):
